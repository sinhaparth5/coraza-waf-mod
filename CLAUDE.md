...
**Module layout**: `config/` (YAML load + defaults), `waf/` (Coraza engine wrapper), `services/` (DB-backed routing registry + TLS), `proxy/` (the request pipeline), `storage/` (SQLite access, schema migrations, async log queue), `blocklist/` (IP rules), `geo/` (GeoIP2 country blocking, MaxMind GeoLite2), `ui/` (admin dashboard handlers + templates), `tools/minify/` (build-time JS minifier, not shipped in the binary).
