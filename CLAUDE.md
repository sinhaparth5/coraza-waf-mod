...
**Daily report email** (`mailer/`): `mailer.NewReporter(db)` (started in `main.go`) sleeps until just past local midnight, then emails a summary of the completed day — total/blocked/403 counts plus a per-stage breakdown from `storage.GetDailyReport` — over SMTP with **implicit TLS only** (SMTPS :465, the sole mode Cloudflare Email Service supports; no STARTTLS). SMTP settings including the API token live in the `meta` table (`storage.EmailConfig`), entered via the Settings page — **the token is never shipped in the repo, binary, config.yaml, or install.sh**. Sends are idempotent per day via the `email_report_sent_for` meta key, so restarts around midnight neither duplicate nor skip a report (a failed send is *not* marked, so the next startup/tick retries). `Reporter.SendNow` (wired into `ui.NewHandler`) powers the "Send test report" button and works with the schedule disabled, using the *saved* config. In the save handler, a blank token field keeps the stored token, and the token is never echoed back into the page. `net/smtp.PlainAuth` works here because `smtp.NewClient` detects the `*tls.Conn` and marks the session TLS.
