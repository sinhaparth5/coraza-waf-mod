...
UNIT
	...
	# The Services page's "Cache performance" card runs varnishstat, which
	# reads varnishd's shared memory — readable by the varnish group only.
	# Restart so the running WAF picks up the new supplementary group.
	if getent group varnish >/dev/null 2>&1 && ! id -nG "${SERVICE_USER}" 2>/dev/null | grep -qw varnish; then
		run usermod -aG varnish "${SERVICE_USER}"
		run systemctl restart "${BINARY_NAME}"
	fi
